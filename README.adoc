...
= 3DStris image:{license-badge}[MIT, link=LICENSE] image:{cpp11-badge}[C\+\+ 11, link={isocpp}] image:{travis-badge}[Travis, link={travis}] image:{codefactor-badge}[CodeFactor, link={codefactor}] image:{downloads-badge}[GitHub release downloads, link={latest-release}]
:toc: preamble
