...
void Board::draw(Vector2 origin, int tileSize) {
	...
	// bottom
	straightLine(origin.x, origin.y + height * tileSize, width * tileSize, 0.0f, outerThick, BLACK);
	// left
	straightLine(origin.x, origin.y, 0.0f, height * tileSize, outerThick, BLACK);
	// right
	straightLine(origin.x + width * tileSize, origin.y, 0.0f, height * tileSize, outerThick, BLACK);

	const int gridThick = 1;

	for (int y = 1; y < height; ++y) {
		straightLine(origin.x, origin.y + y * tileSize, width * tileSize, 0.0f, gridThick, GRIDBLACK);
	}
	for (int x = 1; x < width; ++x) {
		straightLine(origin.x + x * tileSize, origin.y, 0.0f, height * tileSize, gridThick, GRIDBLACK);
	}
